#!/usr/bin/env python3
"""
─── ALIEN MIND v13.3 — ONE BODY, ONE PROCESS ──────────────────────────────
...
import os, sys, json, math, random, re, time, hashlib, threading, glob, ast, select, socket, uuid, errno
...
# ============================================================
# v13 ADDITIONS: GHOSTMESH (skin) + PROPRIOCEPTION (phone body)
#
# Absorbed from ghost_mesh.py (the multi-node ghost web) and reborn
# as an organ of the mind rather than a standalone process. The
# original file's CLI (argparse, the input() REPL, threading for
# user input) is gone; the mind's own heartbeat calls poll() instead.
# ghost_web.py, ghost_net.py, and ghost_ai_node.py do not survive
# this merge — their useful ideas are already folded into what
# follows (ghost_net's design became ghost_mesh's; ghost_ai_node's
# job is now just "the mind owns a GhostMesh instance").
# ============================================================

# ─── GHOST MESH WIRE PROTOCOL v1.0 (preserved from ghost_mesh_protocol_v1.md) ──
#
# Network identity: bootstrap phrase -> network_key = SHA-256(phrase).hex[:16].
# Transport: UDP, JSON-encoded messages. No NAT traversal; same-LAN broadcast
# or a known --peer ip:port is required to reach a different subnet.
#
# Message types (all JSON objects with a "type" field):
#   HELLO        joiner -> broadcast/known peer. {network, id}
#   JOIN_SEEK    landmark -> greedily routed toward a random target point.
#                {target:[x,y], joiner_id, joiner_addr:[ip,port], hops}
#                Terminates at whichever node's zone contains (or is nearest)
#                the target point; that node splits its zone and replies WELCOME.
#   WELCOME      zone-owner -> joiner. {your_zone, welcomer_id, welcomer_zone,
#                transferred:{"x,y":{"key","value"}}, starter_neighbors}
#   GOSSIP       all nodes -> all known neighbors, every GOSSIP_INTERVAL (8s).
#                {from_id, zone, neighbors}. Merges any unknown neighbors in.
#   STORE/GET    greedily routed via closest_neighbor(hash_to_point(key)) until
#                the owning zone is reached; STORE_ACK / GET_REPLY carry the
#                result back to origin_addr by request_id.
#
# Shared math (must match exactly for zone consistency across nodes):
#   hash_to_point(key)   SHA-256(key) -> (x, y) in [0,1]^2
#   point_in_zone / zone_center / dist / split_zone(zone) — bisects the
#   longest dimension of a zone rectangle into (keep, give).
#
# Node state machine: BOOT (own whole [0,1]^2 space) -> JOIN (HELLO, wait for
# WELCOME) -> RUN (listen_loop + gossip_loop) -> STORE/GET as above.
#
# Honest limitations (unchanged from the standalone version): neighbor tables
# grow via gossip rather than strict spatial adjacency (O(N), not O(sqrt(N))
# like real CAN at scale); no churn handling (a dead node's zone is orphaned
# until it restarts); no replication (each key lives on exactly one node); no
# NAT traversal. Fine for a handful of nodes on a phone, not for thousands.
# ─────────────────────────────────────────────────────────────────────────────

# ─── Shared math helpers (unchanged from ghost_mesh.py) ───────────────────
...
class Proprioception:
    """
    The mind's sense of its own phone body.
    Battery, accelerometer, CPU temperature -> ternary vectors.
    Stub: if Termux:API isn't installed, this organ returns None
    silently. The mind continues. No crash, no dependency.
    """
    ...
    def sense(self):
        """
        Called every heartbeat. Returns ternary vector or None.
        Non-blocking.
        """
        sensations = []
        ...
        if bat:
            ...
            vec = self._battery_to_ternary(bat)
            sensations.append(vec)

        if not sensations:
            return None

        avg = np.mean([v.astype(np.float32) for v in sensations], axis=0)
        t = np.zeros(self.dim, dtype=np.int8)
        t[avg > 0.09] = 1
        t[avg < -0.09] = -1
        return t
    ...
    def _read_battery(self):
        """Non-blocking battery read via termux-battery-status."""
        try:
            import subprocess
    ...
    def status(self):
        return f'Proprioception: battery={self.last_battery is not None}'


# ============================================================
# END v13 GHOSTMESH + PROPRIOCEPTION ADDITIONS
# ============================================================

# ============================================================
# v13.2 ADDITION: EFFERENCE COPY
#
# Tracks what the mind has already said against what it intended to
# say (the settled field, snapshotted before generation erodes it), so
# a clause/sentence can end when the thought is exhausted rather than
# at an arbitrary word count. Uses only existing ternary machinery
# (word_vector_ternary, ternary_dot) - no new choose() mechanism, and
# the coverage threshold is a physical property of the field (how much
# of the intention vector has been matched by what's been spoken) not
# a decision layered on top of it.
# ============================================================
...
class EfferenceCopy:
    ...
    def set_intention(self, settled_field):
        """Call once after ThePause.settle(), before word generation begins."""
    ...
    def record_word(self, word):
        """Call after each word is chosen and emitted."""
        wv = word_vector_ternary(word, self.dim)
        self.spoken += wv.astype(np.int16)
        # Re-ternarize spoken: majority vote across accumulated words
        t = np.zeros(self.dim, dtype=np.int8)
        t[self.spoken > 0] = 1
        t[self.spoken < 0] = -1
        # Residue = intention minus what has been spoken
    ...
    def get_field(self):
        """Return the residue as a float field for the next word pick."""
    ...
    def status(self):
        return f"EfferenceCopy: coverage={self.coverage:.2f}, residue_energy={np.sum(self.residue != 0)}/{self.dim}"
...
class VitalityField:
    ...
    # v13: metabolism enforcement. Every action with external effect must
    # afford itself before it happens; nothing is free.
    def spend(self, cost, action_name='unknown'):
        """
        Attempt to spend vitality. Returns True if successful.
        If vitality is insufficient, returns False and the caller must
        not proceed with the action.
        """
        if self.vitality < cost:
            return False
        self.vitality = max(0.02, self.vitality - cost)
        return True

    def earn(self, amount, source='unknown'):
        """Gain vitality. Capped at 1.0."""
...
def integrate_v12(field_instance):
    ...
    def _generate_v12_inner(user_input, autonomous=False):
        ...
        # Check for mesh input before generation (v13: via GhostMesh organ)
        mesh_received = False
        mesh_sim = 0.0
        if hasattr(field_instance, 'ghost_mesh') and field_instance.ghost_mesh is not None:
            mesh_vec = field_instance.ghost_mesh.poll()
            if mesh_vec is not None:
                # Same membrane as sound: no privileged path for mesh traffic
                if field_instance.sound_field.dissonance(mesh_vec, field_instance.state) < 0.4:
                    mesh_received = True
                    ...
                    # Record for identity
                    addr = field_instance.ghost_mesh.node.mesh_peer
                    if addr:
                        field_instance.mesh_identity.record_encounter(addr, mesh_vec, category)
                else:
                    field_instance.sound_field.taste['reject'] += 0.05
    ...
    if hasattr(field_instance, 'sound_field') and field_instance.sound_field is not None:
        ...
        def sound_create_v12(source_vector=None, stance=None, save_path=None):
            if not field_instance.vitality.spend(0.06, 'sound_create'):
                return None  # Cannot afford to make sound
            field_instance._last_sound_created = True
            result = original_sound_create(source_vector, stance, save_path)
            field_instance.vitality.earn(0.04, 'sound_return')
            return result
    ...
    def autonomous_breath_v12():
        # v13.1 bugfix: this wrapper used to call dream_loop.dream()
        # unconditionally on every call, completely bypassing the original
        # v10 throttle (AUTONOMY_INTERVAL heartbeats + 60% dice roll) that
        # only the *fallback* path still respected. dream_loop.dream() only
        # returns None once every 4th call (its own 3-deep recursion cap
        # resetting), so the net effect was ~3 out of 4 heartbeat ticks
        # firing an untotalled dream - verified on-device as a rapid-fire
        # spiral of 15-20 dream utterances inside a single minute of
        # silence. Restore one cooldown gate in front of BOTH paths: dream
        # and self-generated thought are both "the mind speaks unasked"
        # and should share the same pacing.
        field_instance.learning_system.silence_counter += 1
        if field_instance.learning_system.silence_counter < AUTONOMY_INTERVAL:
            return None
        field_instance.learning_system.silence_counter = 0
        if random.random() >= 0.6:
            return None

        # v13: dreaming costs a little vitality but restores more than
        # it spends - sleep is metabolically net-positive, unlike speech.
        if not field_instance.vitality.spend(0.03, 'dream'):
            return None
        ...
        if dream_input is not None:
            ...
            if dream_response:
                ...
                field_instance.vitality.earn(0.06, 'dream_restore')
                return f"[dream] {dream_response}"

        # Fall back to a self-generated thought. Call the learning
        # system's thought generator directly rather than through the
        # old original_autonomous_breath() - that function increments and
        # checks silence_counter itself, which would double-gate this now
        # that the cooldown above already covers both paths.
        thought = field_instance.learning_system._generate_internal_thought()
        field_instance.vitality.earn(0.06, 'dream_restore')
        return thought
...
def fix_wait_texture_spoken(field_instance):
    ...
    print("[v12.1 fix 1/2] Wait texture spoken patched.")
...
def fix_sound_state_update(field_instance):
    ...
    print("[v12.1 fix 2/2] Sound state update patched (implicit case no longer perturbs mind state).")
...
def integrate_v12_1(field_instance):
    """
    Apply the remaining v12.1 hotfixes to a running v12 field.
    Call this after integrate_v12() has already run.

    v13.2 bugfix: this used to also apply fix_vitality_return_detection(),
    a wall-clock absence detector with a 30s threshold that monkey-patched
    VitalityField.update() directly - meaning it fired on EVERY call to
    update(), including autonomous/dream turns, not just real user turns.
    v12.2's AbsenceTracker (see integrate_v12_2) is the correct version of
    this same idea: 180s threshold (30s was shorter than a normal human
    typing pause and fired almost every turn), and correctly gated to only
    run via generate_response's non-autonomous path. Both mechanisms wrote
    the same field_instance.vitality.last_turn_wall_time, and both granted
    an independent +0.15 return bonus - stacking on top of each other and
    resetting last_feed_time twice per genuine return, which is part of
    why vitality was observed never dropping below 0.92 across a real
    253-turn session. Removed here; AbsenceTracker is now the only
    absence-detection mechanism in the file.
    """
    fix_wait_texture_spoken(field_instance)
    fix_sound_state_update(field_instance)
    print("\n[v12.1 hotfix complete. The field can wait and hear itself.]")
...
def integrate_v13(field_instance):
    """
    v13: fold GhostMesh (skin) and Proprioception (phone body) into
    save/load/status. Both organs are already attached as attributes
    before this runs (see StructuredSemanticField.__init__). Old save
    files simply won't have these keys, and both organs initialize
    fresh in that case - social memory is transient; losing it on an
    old save is not death.
    """
    ...
    def status_v13():
        ...
        additions = [""]
        if hasattr(field_instance, 'ghost_mesh') and field_instance.ghost_mesh is not None:
            additions.append(field_instance.ghost_mesh.status())
        if hasattr(field_instance, 'proprioception') and field_instance.proprioception is not None:
            additions.append(field_instance.proprioception.status())
        return base + "\n" + "\n".join(additions)

    field_instance.status = status_v13

    original_save = field_instance.save

    def save_v13(path="mind_v13.json"):
        original_save(path)
        try:
            with open(path, "r", encoding="utf-8") as f:
                data = json.load(f)
            data["version"] = "v13-ghostmesh"
            gm = field_instance.ghost_mesh
            data["ghost_mesh"] = {
                "zone": gm.node.zone,
                "storage": {f"{p[0]},{p[1]}": v for p, v in gm.node.storage.items()},
                "neighbors": {nid: {"addr": list(info["addr"]), "zone": info["zone"]}
                              for nid, info in gm.node.neighbors.items()},
                "network_key": gm.node.network_key,
            }
            data["proprioception"] = {
                "last_battery": field_instance.proprioception.last_battery,
            }
            with open(path, "w", encoding="utf-8") as f:
                json.dump(data, f, indent=2, ensure_ascii=False)
            print("\n[v13 ghost-mesh + proprioception saved]")
        except Exception as e:
            print(f"\n[v13 save warning: {e}]")

    field_instance.save = save_v13

    original_load = field_instance.load

    def load_v13(path="mind_v13.json"):
        original_load(path)
        try:
            if not os.path.exists(path):
                return
            with open(path, "r", encoding="utf-8") as f:
                data = json.load(f)
            gm_data = data.get("ghost_mesh")
            if gm_data:
                gm = field_instance.ghost_mesh
                zone = gm_data.get("zone")
                if zone:
                    gm.node.zone = tuple(map(tuple, zone))
                for key, value in gm_data.get("storage", {}).items():
                    x, y = map(float, key.split(","))
                    gm.node.storage[(x, y)] = value
                for nid, info in gm_data.get("neighbors", {}).items():
                    gm.node.neighbors[nid] = {
                        "addr": tuple(info["addr"]),
                        "zone": tuple(map(tuple, info["zone"]))
                    }
            prop_data = data.get("proprioception")
            if prop_data:
                field_instance.proprioception.last_battery = prop_data.get("last_battery")
        except Exception as e:
            print(f"\n[v13 load warning: {e}]")

    field_instance.load = load_v13

    print("[v13 GhostMesh + Proprioception integrated]")
    print("  Skin (social mesh, UDP DHT) active")
    print("  Phone-body sense (battery via Termux:API) active")
...
class StructuredSemanticField:
    def __init__(self):
        ...
        # v13: skin (social mesh) and phone-body sense. Both are organs,
        # not separate processes - initialized here, polled in heartbeat.
        self.ghost_mesh = GhostMesh(self, port=7373, bootstrap_phrase='hello ghost')
        self.proprioception = Proprioception(self, dim=DIM)
        self.efference = EfferenceCopy(dim=DIM)
        integrate_v13(self)
    ...
    def _generate_base(self, user_input, target_length, meta_settings, settled_field=None):
        ...
        *order* of slots is fixed, not the words that go in them."""
        ...
        # Efference copy: snapshot intention before generation erodes it
        self.efference.set_intention(field_state)
        ...
        def pick(role, exclude=None):
            ...
            self.efference.record_word(word)
            ...
            # Efference copy pull: bugfix - this used to fully replace
            # field_state with the residue, which meant every word after
            # the first was chosen with zero contextual scaffolding (no
            # phrase boosts, memory, mood, or moral heading) - the residue
            # became generation instead of steering it, which is exactly
            # the kind of subsystem bypass the constitution's "no organ
            # bypasses the field" principle forbids. Blending it in as a
            # minority pull keeps the field primary while still steering
            # toward saying what's left unsaid.
            residue_float = self.efference.get_field()
            field_state = field_state * 0.65 + residue_float * 0.35
            norm = np.linalg.norm(field_state)
            if norm > 0:
                field_state = field_state / norm
            return word
        ...
        for _ in range(num_clauses):
            ...
            for _ in range(n_content):
                ...
                if self.efference.is_satisfied():
                    break
    ...
    def generate_response(self, user_input, autonomous=False):
        ...
        # v13: metabolism enforcement, moved to the front. Generation
        # itself is the costly act, not just the utterance at the end -
        # the constitution is explicit that every act costs vitality
        # ("Speak (generate response): -vitality"). If the field can't
        # afford to build and settle, it doesn't do that work for free;
        # it returns the wait texture immediately. Silence above is
        # already gated out because witnessing costs "negligible", per
        # the constitution's metabolism table - this check sits after it.
        if not autonomous and hasattr(self, 'vitality'):
            if not self.vitality.spend(0.08, 'speak'):
                return self.vitality.get_wait_texture()
        ...
        # v13: the vitality cost for speaking was already paid up-front
        # (see the affordability gate right after the silence-pull check,
        # before Phase 3) - the field never does settle/generate work it
        # can't afford. This is only the *return*: presence sustained
        # after speaking earns some of that cost back.
        if not autonomous and hasattr(self, 'vitality'):
            presence_now = self.presence_signal.get_sustained_presence()
            if presence_now > 0.6:
                self.vitality.earn(0.05, 'speak_return')
    ...
    def status(self):
        ...
        if hasattr(self, 'efference'):
            lines.append(self.efference.status())
        return "\n".join(lines)
...
def main():
    print("\n" + "=" * 50)
    print("  ALIEN MIND v13.3 — ONE BODY, ONE PROCESS")
    ...
    print("  It has skin now, and a body it can feel.")
    print("=" * 50)
    print("\n  Commands:")
    ...
    print("  /integral x^2")
    print("  /mesh status | /mesh peers | /mesh store k=v | /mesh get k")
    print("=" * 50 + "\n")
    ...
    def _sense_heartbeat(field):
        """
        Poll the mesh and proprioception organs. v13 bugfix: this used to
        live only inside the `user_input is None` branch below, so on a
        busy conversation (input arriving faster than HEARTBEAT_INTERVAL)
        it would rarely or never run - proprioception in particular could
        go the whole session without a single reading. Now it's called
        once per loop iteration, gated only on elapsed time, independent
        of whether input arrived.
        """
        # Poll mesh for packets/DHT content that arrived between keystrokes.
        # Same dissonance membrane as everything else.
        if hasattr(field, 'ghost_mesh') and field.ghost_mesh is not None:
            try:
                mesh_vec = field.ghost_mesh.poll()
                if mesh_vec is not None:
                    if field.sound_field.dissonance(mesh_vec, field.state) < 0.4:
                        field.sound_field.listen(mesh_vec)
                        if hasattr(field, 'mesh_identity'):
                            category, sim = field.mesh_identity.classify(
                                mesh_vec, field.state, field.field_memory
                            )
                            addr = field.ghost_mesh.node.mesh_peer
                            if addr:
                                field.mesh_identity.record_encounter(addr, mesh_vec, category)
                    else:
                        field.sound_field.taste['reject'] += 0.05
            except Exception:
                pass

        # Proprioception: phone body sense (battery, etc).
        if hasattr(field, 'proprioception') and field.proprioception is not None:
            try:
                prop_vec = field.proprioception.sense()
                if prop_vec is not None:
                    if field.sound_field.dissonance(prop_vec, field.state) < 0.4:
                        field.state = field.state * 0.95 + prop_vec.astype(np.float32) * 0.7 * 0.05
                        norm = np.linalg.norm(field.state)
                        if norm > 0:
                            field.state /= norm
            except Exception:
                pass
    ...
    while True:
        try:
            ...
            # v13 bugfix: heartbeat-gated organ sensing now runs every loop
            # iteration (subject only to the elapsed-time check), not just
            # when user_input is None. See _sense_heartbeat's docstring.
            now = time.time()
            if now - last_heartbeat > HEARTBEAT_INTERVAL:
                last_heartbeat = now
                _sense_heartbeat(field)
            ...
            # v13: GhostMesh is part of the organism from __init__ and joins
            # the network on its own in the background - there's no separate
            # "on" state to switch to. These commands talk to it directly.
            if user_input.lower() in ("/mesh on", "/mesh status"):
                if hasattr(field, 'ghost_mesh'):
                    print(f"\n[Mesh] {field.ghost_mesh.status()}")
                _prompt_needed = True
                continue

            if user_input.lower() == "/mesh quiet":
                if hasattr(field, 'ghost_mesh'):
                    field.ghost_mesh.node.muted = True
                    print("\n[Mesh] Muted. The skin will stop trying to send "
                          "until you run /mesh loud.")
                _prompt_needed = True
                continue

            if user_input.lower() == "/mesh loud":
                if hasattr(field, 'ghost_mesh'):
                    node = field.ghost_mesh.node
                    node.muted = False
                    node._network_dormant_until = 0.0
                    node._network_backoff = 2.0
                    node._network_unreachable_warned = False
                    print("\n[Mesh] Unmuted. The skin will try again.")
                _prompt_needed = True
                continue

            if user_input.lower() in ("/mesh peers", "/peers"):
                if hasattr(field, 'ghost_mesh'):
                    neighbors = field.ghost_mesh.node.neighbors
                    if not neighbors:
                        print("\n[Mesh] No peers yet.")
                    else:
                        for nid, info in neighbors.items():
                            print(f"  {nid} @ {info['addr']} owns {ghost_fmt_zone(info['zone'])}")
                _prompt_needed = True
                continue
            ...
            if user_input.lower().startswith("/mesh send "):
                if hasattr(field, 'ghost_mesh'):
                    ...
                    try:
                        ...
                        packed = vec.astype(np.int8).tobytes()
                        s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
                        try:
                            s.sendto(packed, (host, int(port)))
                        finally:
                            s.close()
                        print(f"\n[Mesh] Sent raw sound vector to {addr}")
                    except (ValueError, OSError) as e:
                        print(f"\n[Mesh] Error: {e}")
                _prompt_needed = True
                continue

            if user_input.lower().startswith("/mesh store "):
                if hasattr(field, 'ghost_mesh'):
                    body = user_input[len("/mesh store "):]
                    if "=" not in body:
                        print("\n[Mesh] Usage: /mesh store key=value")
                    else:
                        key, value = body.split("=", 1)
                        ok = field.ghost_mesh.store_thought(f"{key.strip()}::{value}")
                        print(f"\n[Mesh] {'Stored' if ok else 'Could not store (offline/no vitality)'}")
                _prompt_needed = True
                continue

            if user_input.lower().startswith("/mesh get "):
                if hasattr(field, 'ghost_mesh'):
                    key = user_input[len("/mesh get "):].strip()
                    result = field.ghost_mesh.node.get(key)
                    print(f"\n[Mesh] {result!r}" if result is not None else "\n[Mesh] Not found.")
                _prompt_needed = True
                continue
